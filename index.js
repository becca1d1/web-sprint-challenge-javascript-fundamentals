...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
//The variable internal is block scoped, which means that nested funciton can access it since it is in the same block
...
//   create variable to be the counter
//  create a for loop that loops up to number and includes number (<=)
//     add the incrementing value (i) to the sum
function summation(number){
  ...
  console.log('counter function:', summation(4));
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(zooArray){
    const displayNames = [];
    zooArray.forEach(element => {
      displayNames.push(`name: ${element.animal_name}, scientific: ${element.scientific_name}`);
    });
    return displayNames
  }
  console.log('animalNames is returning this:',animalNames(zooAnimals));
  ...
  */
  ...
  function lowerCaseNames(zooArr){
    const zooAr = zooArr.map((element) => {
  ...
  */
  ...
  function lowPopulationAnimals(zooA){
    const filterArr = zooA.filter((element) => {
      return element.population < 5;
    })
    return filterArr;
  }
  console.log('Animals with low population:' ,lowPopulationAnimals(zooAnimals));
  ...
  */
  ...
  function USApop(zooAl){
    const reducedArr = zooAl.reduce((accumulator, item) => {
      return accumulator + item.population;
    }, 0);
    return reducedArr
  }
  console.log('Zoo Animals total population:', USApop(zooAnimals));
...
function multiply(num3, num4){
  ...
  // console.log('multiply two numbers:', multiply(3, 4));
...
function greeting(firstName, lastName){
  ...
  // console.log('Hello first and last name:', greeting('becca', 'shum'));
